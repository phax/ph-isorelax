 ...
 * <p>
 * An instance of this interface can be obtained through the {@link VerifierFactory#newVerifier}
 * method or {@link Schema#newVerifier} method. Once it is created, an application can use one
 * instance to validate multiple documents.
 * <p>
 * This interface is <b>not thread-safe</b> and <b>not reentrant</b>. That is, only one thread can
 * use it at any given time, and you can only validate one document at any given time.
 *
 * @since Feb. 23, 2001
 * @version Mar. 4, 2001
...
public interface Verifier
{
   ...
   * <p>
   * Now a verifier implementation is <b>required</b> to support VerifierHandler. Therefore an
   * application doesn't need to check this feature.
   *
   * @deprecated
   */
  @Deprecated
   ...
   * <p>
   * Now a verifier implementation is <b>required</b> to support VerifierFilter. Therefore an
   * application doesn't need to check this feature.
   *
   * @deprecated
   */
   ...
   * <p>
   * If no error handler is set explicitly, a verifier implementation will not report any
   * error/warning at all. However, the caller can still obtain the result of validation through the
   * return value.
   * <p>
   ...
   * <p>
   * The given entity resolver is used in the {@link #verify(String)} method and the
   ...
   * validates an XML document.
   * <p>
   * An implementation is required to accept <code>Document</code> object as the node parameter. If
   * it also implements partial validation, it can also accepts things like <code>Element</code>.
   *
   * @param node
   *        the root DOM node of an XML document.
   * @exception UnsupportedOperationException
   *            If the node type of the node parameter is something which this implementation does
   *            not support.
   * @return <b>true</b> if the document is valid. <b>false</b> if otherwise.
   */
   ...
   * <p>
   * you can use the returned <code>VerifierHandler</code> to validate documents through SAX.
   * <p>
   * Note that two different invocations of this method can return the same value; this method does
   * NOT necessarily create a new <code>VerifierHandler</code> object.
   */
   ...
   * <p>
   * Note that two different invocations of this method can return the same value; this method does
   * NOT necessarily create a new <code>VerifierFilter</code> object.
   */
